import { encodeMsg, decodeMsg } from './msgCode'
import { CMD as COMMAND, PACKET as PACKETENUM } from './const'
...
export default class IM {
  ...
  _onmessage(event) {
    ...
    this.handleResponseEvent(JSON.parse(msg))
    // 超时定时器
  ...
  _heartBeat(immediate) {
    this.pingTimer = setTimeout(
      () => {
        this._sendMessage({ command: COMMAND.HEARTBEAT_REQUEST, data: {} })
        this._heartBeat()
      },
  ...
  _sendMessage(msg) {
    if (this.socket.readyState === WebSocket.OPEN) {
      msg.version = PACKETENUM.VERSION
      msg.algorithm = PACKETENUM.SERIALIZE
      this.socket.send(encodeMsg(JSON.stringify(msg)))
    }
  }
